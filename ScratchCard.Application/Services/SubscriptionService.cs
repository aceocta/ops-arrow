...
public class SubscriptionService : ISubscriptionService
{
    ...
    public async Task<EntitlementsDto> GetEntitlementsAsync(Guid companyId, CancellationToken cancellationToken = default)
    {
        var summary = await GetSummaryAsync(companyId, cancellationToken);

        var status = summary.Status;
        var isInTrial = status == SubscriptionStatus.TrialActive || (summary.TrialDaysRemaining ?? 0) > 0;
        var isActive = status == SubscriptionStatus.Active || isInTrial;
        var inGracePeriod = status == SubscriptionStatus.PastDue || status == SubscriptionStatus.PaymentFailed;
    ...
    public async Task<SubscriptionSummaryDto> RecordIapReceiptAsync(IapReceiptRequest request, CancellationToken cancellationToken = default)
    {
        // STUB: in-app purchase receipt validation will be wired to the chosen IAP provider
        // (App Store StoreKit / Google Play Developer API / RevenueCat).
        // For now we persist a billing event for audit and return the current summary so the
        // mobile client continues to function. Production must:
        //   1. Verify the receipt against the store (or trust the RevenueCat webhook below).
        //   2. Resolve the product SKU to a SubscriptionPlan.
        //   3. Activate/extend the CompanySubscription accordingly.
        ...
        var company = await _companyRepository.GetByIdAsync(request.CompanyId, cancellationToken)
            ?? throw new AppException("not_found", "Company not found.", 404);

        var activeSubscription = await _companySubscriptionRepository.Query()
        ...
        var billingEvent = new BillingEvent
        {
            ...
            CompanySubscriptionId = activeSubscription?.Id,
            EventType = BillingEventType.IapReceiptReceived,
            Description = $"IAP receipt received from {request.Platform} for product {request.ProductId}.",
        ...
        return await GetSummaryAsync(company.Id, cancellationToken);
    }

    public async Task HandleIapWebhookAsync(IapWebhookEvent webhookEvent, CancellationToken cancellationToken = default)
    {
        // STUB: RevenueCat / App Store Server Notifications / Google Real-time Developer Notifications
        // call this endpoint with subscription lifecycle events. Persist for audit; production must
        // map AppUserId -> CompanyId and update CompanySubscription status / period dates.
        if (!Guid.TryParse(webhookEvent.AppUserId, out var companyGuid))
        {
        ...
        {
            // Without a resolvable company we cannot persist a BillingEvent (CompanyId is required);
            // production must dead-letter the message for manual review. Returning silently here so
            // the webhook endpoint can ack and the provider does not retry indefinitely.
            return;
        }
        ...
        var billingEvent = new BillingEvent
        {
            ...
            EventType = BillingEventType.IapWebhookEvent,
            Description = $"{webhookEvent.Provider}/{webhookEvent.EventType} for product {webhookEvent.ProductId}.",
