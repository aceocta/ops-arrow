...
Ops Arrow is a multi-tenant operations platform for shops, forecourts and small businesses. It supports:

- Staff and shift management, time-clock and attendance.
- Scratch-card pack reconciliation and sales reporting.
- Till report capture with OCR.
- Delivery note recording with AI-assisted parsing.
- Compliance check-ins, temperature logs, refusal registers, and visitor sign-in books.
- Day-end cash reconciliation and financial reporting.
- Team notifications by push, email and (optionally) WhatsApp.
...
- **Crash / performance diagnostics:** anonymous aggregate telemetry only (no third-party crash reporter is currently integrated; debug logging is disabled in production builds).
...
| **Google Firebase** (Cloud Messaging) | Push notification token issuance and delivery | US |
| **Meta Platforms Ireland Ltd** (WhatsApp Business Cloud API) | Optional WhatsApp message delivery | EU / US |
| **Brevo (Sendinblue)** | Transactional and marketing email delivery | EU (France) |
| **Hostinger** | Backup SMTP email delivery | EU |
...
- Passwords are stored as **salted PBKDF2 hashes** via ASP.NET Core Identity; signup verification codes and password-reset tokens are also stored as one-way hashes. We never see, log or store plain-text credentials.
- API access requires a JSON Web Token bearer credential issued only after successful authentication; tokens are short-lived and refreshed.
- On mobile, access tokens are stored in the operating system's secure keystore (**iOS Keychain** / **Android EncryptedSharedPreferences**) via `expo-secure-store`.
...
- Subscription payments are processed entirely by Stripe / Apple / Google — we never see your full card number.
...
- The access token in the OS secure keystore (iOS Keychain / Android EncryptedSharedPreferences) via `expo-secure-store`.
...
|---|---|---|
| **Camera** | Capture photographs of till reports, delivery notes, shift attachments, visitor photographs, and to scan barcodes | Yes — only requested when you tap a camera button |
...
- **Push:** delivered via Apple Push Notification service (APNs, iOS) or Google Firebase Cloud Messaging (FCM, Android). You can disable push at any time in your device settings; you can also remove a specific device's token by signing out on that device.
- **Email:** sent by Brevo (transactional and marketing) or Hostinger SMTP (fallback for transactional). Marketing emails include an unsubscribe link.
