...
import { getCurrentUserProfile, signInWithDevBypass, signInWithPassword as signInWithPasswordApi, signUpWithPassword as signUpWithPasswordApi } from "../api/authApi";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function signInWithDevBypassLogin(payload: { email?: string; firstName?: string; lastName?: string; role?: string; shopId?: string }) {
    ...
    try {
      ...
      const nextShopId = resolveActiveShopId(result.profile, payload.shopId);
  ...
  async function refreshProfile() {
    const currentUser = await getCurrentUserProfile();
    const nextShopId = resolveActiveShopId(currentUser, activeShopId);
    setProfile(currentUser);
    setActiveShopId(nextShopId);
  ...
  async function applyAuthTokenResult(result: { accessToken: string; profile: AuthProfile }, preferredShopId?: string | null) {
    await saveAccessToken(result.accessToken);
    const nextShopId = resolveActiveShopId(result.profile, preferredShopId);
    setProfile(result.profile);
    setActiveShopId(nextShopId);
