...
import { DateTimeField, formatDateValue, formatTimeValue, parseDateValue } from "../../components/DateTimeField";
...
export function TemperatureLogScreen() {
  ...
  const [historyFrom, setHistoryFrom] = useState(shiftDate(selectedDate, -6));
  const [historyTo, setHistoryTo] = useState(selectedDate);

  useEffect(() => {
    setHistoryTo(selectedDate);
    setHistoryFrom(shiftDate(selectedDate, -6));
  }, [selectedDate]);
  ...
  const historyQuery = useQuery({
    queryKey: ["temperature-reading-history", shopId, historyFrom, historyTo],
    queryFn: () => listTemperatureReadings(shopId as string, historyFrom, historyTo),
    enabled: Boolean(shopId) && historyFrom.length === 10 && historyTo.length === 10,
  });
  ...
  const recordMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["temperature-reading-history", shopId, historyFrom, historyTo] }),
      ]);
    },
    onError: (error: any) => {
  ...
  const selectedUnitLatestReading =
    selectedUnitLog && selectedUnitLog.readings.length > 0
      ? selectedUnitLog.readings[selectedUnitLog.readings.length - 1]
      : undefined;
  useEffect(() => {
    if (!selectedUnit) {
      return;
            ...
            {dailyUnitLogs.map((unitLog) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={styles.unitRowActionHint}>Tap to log reading</Text>
                </Pressable>
              );
            })}
          ...
          <View style={styles.modalBackdrop}>
            <View style={styles.modalCard}>
              ...
              <Text style={styles.sectionTitle}>
                Log Reading: {selectedUnit?.unitName ?? "Unit"}
              </Text>
              {selectedUnit ? (
                <View style={styles.rowBetween}>
                  <Text style={styles.meta}>
              ...
              <View style={styles.entryRow}>
                ...
                >
                  <Text style={styles.noteActionLabel}>Notes (Optional)</Text>
                ...
                >
                  ...
                  <Text style={[styles.noteActionValue, !actionTaken.trim() ? styles.noteActionPlaceholder : null]} numberOfLines={2}>
                    {actionTaken.trim() || "Tap to add corrective action"}
                  </Text>
                </Pressable>
              </View>
        ...
        >
          <View style={styles.modalBackdrop}>
            <View style={styles.modalCard}>
              ...
              <Text style={styles.meta}>
                {textEditorField === "notes"
                  ? "Add notes for this reading."
                  : "Describe corrective action. This is required when out of range."}
              </Text>
              <TextInput
